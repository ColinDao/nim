...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        actions_eval = self.actions_eval(state)
    ...
    def actions_eval(self, state):
        """
        Given a state `state`, return all the available actions
        `(i, j)` and their corresponding Q-value. 
        
        If an action does not have a Q-value, set it to 0.
        """
        available_actions = {}
        ...
        for i in range(len(state)):
            ...
            # Loop through each amount that can be taken
            for j in range(1, state[i] + 1):

                # Assign corresponding Q-value if one exists, else 0
                action = (i, j)
                q = self.q[tuple(state), action] if (tuple(state), action) in self.q else 0
                available_actions[action] = q
